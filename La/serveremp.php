<?php 
	...
	if (isset($_POST['reg_emp'])) {
        ...
        $jtype = mysqli_real_escape_string($db, $_POST['jtype']);
        ...
        if (empty($jtype)) { array_push($errors, "Job Type is required"); }
		...
		if (count($errors) == 0) {
			...
			$query = "INSERT INTO employ (username,gender,nid, tnum,stnum,psadd,pradd,parea,jtype,email,password) 
					  VALUES('$username','$gender','$nid','$tnum','$stnum','$psadd','$pradd','$parea','$jtype','$email', '$password')";
	...
	if (isset($_POST['login_user'])) {
		...
		if (count($errors) == 0) {
			...
			$query = "SELECT * FROM employ WHERE username='$username' AND password='$password'";
